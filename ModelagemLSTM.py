# ModelagemLSTM - IA em Ativos Financeiros para Previsão de Lucro por Ação (EPS)

# Como solução usaremos um modelo de IA com arquitetura LSTM (Long Short-Term Memory)
...
# Carrega os dados
df_modelagem = pd.read_csv('dataset.csv')

# Define nossa série temporal
eps = df_modelagem['eps'].values.reshape(-1, 1)
...
def cria_dataset_lstm(data, look_back = 1):
...
look_back = 1
X_treino, y_treino = cria_dataset_lstm(dados_treino_norm, look_back)
X_teste, y_teste = cria_dataset_lstm(dados_teste_norm, look_back)
...
# Este índice será utilizado para associar cada ponto de dado de treino ao seu ano correspondente no DataFrame de entrada.
original_train_data_index = df_modelagem['ano'][look_back:look_back + len(y_treino_rescaled[0])]
...
original_test_data_index = df_modelagem['ano'][len(y_treino_rescaled[0]) + 2 * look_back:len(y_treino_rescaled[0]) + 2 * look_back + len(y_teste_rescaled[0])]

# Cria um índice para os valores previstos em treino, começando em 'look_back' e terminando em 'look_back + len(previsao_treino)'.
# Este índice será utilizado para associar cada ponto previsto no conjunto de treino ao seu ano correspondente no DataFrame de entrada.
predicted_train_data_index = df_modelagem['ano'][look_back:look_back + len(previsao_treino)]

# Cria um índice para os valores previstos em teste.
predicted_test_data_index = df_modelagem['ano'][len(y_treino_rescaled[0]) + 2 * look_back:len(y_treino_rescaled[0]) + 2 * look_back+len(previsao_teste)]
